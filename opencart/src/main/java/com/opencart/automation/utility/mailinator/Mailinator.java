...
import com.opencart.automation.utility.mailinator.Email.EmailPart;
...
public class Mailinator {
	...
	private static final String MAILINATOR_EMAIL_TEMPLATE_URL = MAILINATOR_API_ENDPOINT + "/message?id=%s&token=%s&private_domain=%b";
	...
	private static final String MAILINATOR_DELETE_ALL_TEMPLATE_URL = MAILINATOR_API_ENDPOINT + "/delete?token=%s&delete_all=%b&private_domain=%b";
	...
	/**
	 * Retrieves all messages from an inbox for a public or private Mailinator
	 * domain
	 *
	 * @param apikey
	 *            - Mailinator API key
	 ...
	 * @param privateDomain
	 *            - indicates if the request is to an account with a private domain
	 * @return List of messages from the inbox
	 * @throws IOException
	 *             When invalid response, for example using an invalid API key
	 */
	public static List<InboxMessage> getInboxMessages(String apikey, String emailAddress, boolean privateDomain)
			throws IOException {
	...
	/**
	 * Once you have the email id's from a given inbox query, you can retrieve the
	 * full email. You can use this for public Mailinator domains only.
	 * @param apikey  - Mailinator API key
	 * @param emailId- ID of email to get, can be retrieved using the {@link #getInboxMessages(String, String)} method
	 ...
	 * You can use either a public or a private Mailinator domain.
	 * @param apikey - Mailinator API key
	 * @param emailId  - ID of email to get, can be retrieved using the {@link #getInboxMessages(String, String, boolean)} method
	...
	public static ArrayList<String> getEmailBody(String apikey, String emailId, boolean privateDomain)
			throws IOException {
	...
	/**
	 * Once you have the email id's from a given inbox query, you can delete the
	 * email. You can use this for either public or private Mailinator domains.
	 * @param apikey - Mailinator API key
	 * @param emailId - ID of email to get, can be retrieved using the {@link #getInboxMessages(String, String, boolean)} method
	 ...
	 * @throws IOException
	 */
	public static boolean deleteAllEmails(String apikey, boolean delete_all, boolean privateDomain) throws IOException {

		String emailUrl = String.format(MAILINATOR_DELETE_ALL_TEMPLATE_URL, apikey, delete_all, privateDomain);
		Reader reader = getResponse(emailUrl);
	...
	/**
	 * Creates a message object based on the JSON representing a message from the
	 * inbox response.
	 *
	 * @param jsonInboxMsg
	 * @return
	...
	private static InboxMessage createInboxMessageFrom(JSONObject jsonInboxMsg) {
		...
		message.setTime(Long.parseLong(jsonInboxMsg.get("seconds_ago").toString()));
	...
	private static Email createEmailFrom(JSONObject jsonEmail) {
		...
		emailMsg.setId(jsonDataSection.get("requestId").toString());
	...
	private static ArrayList<String> getEmailBody(JSONObject jsonEmail) {
		ArrayList emailBody = new ArrayList();
		...
		for (Object jsonPart1 : jsonParts) {
			JSONObject jsonPart = (JSONObject) jsonPart1;
			emailBody.add(jsonPart.get("body").toString());
